...
public static class Layout {
	public static IReadOnlyList<Rect> SplitVertical(Rect area, ReadOnlySpan<Constraint> constraints, int gap = 0, int margin = 0) {
		...
		foreach (var c in constraints) {
			...
			result.Add(inner with { X = cursorX, Width = w });
			cursorX += w + gap;
		}
		return result;
	}
	...
	public static IReadOnlyList<Rect> SplitHorizontal(Rect area, ReadOnlySpan<Constraint> constraints, int gap = 0, int margin = 0) {
		...
		int cursorY   = inner.Y;
		foreach (var c in constraints) {
			int h = c.Kind == ConstraintKind.Length
				? c.Value
				: (int)Math.Floor(remaining * (c.Value / (double)Math.Max(1, pctSum)));

			result.Add(inner with { Y = cursorY, Height = h });
			cursorY += h + gap;
		}
		return result;
	}
	...
	public static IReadOnlyList<Rect> SplitColumnsEven(Rect area, int count, int gap = 0, int margin = 0) {
		...
		for (int i = 0; i < count; i++)
			cs[i] = Constraint.Percentage(pct);
	...
	public static IReadOnlyList<Rect> SplitRowsEven(Rect area, int count, int gap = 0, int margin = 0) {
		...
		var pct = 100 / Math.Max(1, count);
		Span<Constraint> cs = count <= 16
			? stackalloc Constraint[Math.Max(1, count)]
